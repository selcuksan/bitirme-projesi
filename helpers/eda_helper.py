...
from helpers.dataPreprocessing_helper import outlier_thresholds
...
def target_summary_with_num(dataframe, target, numerical_col, target_type="num"):
    if target_type == "num":
        try:
            print(dataframe.groupby(pd.qcut(x=dataframe[target], q=4)).agg(
                {numerical_col: "mean"}), end="\n\n\n")
        except ValueError:
            print(
                dataframe.groupby(pd.qcut(x=dataframe[target].rank(
                    method="first"), q=4)).agg({numerical_col: "mean"}),
                end="\n\n\n")
    else:
